...
use cyclors::{
    ...
    dds_take, DDS_BUILTIN_TOPIC_DCPSPUBLICATION, DDS_BUILTIN_TOPIC_DCPSSUBSCRIPTION,
};
...
pub(crate) fn run_discovery(dp: dds_entity_t, tx: flume::Sender<DiscoveryEvent>) {
    unsafe {
        let _pr = {
...
unsafe extern "C" fn on_data(dr: dds_entity_t, arg: *mut c_void) {
    ...
    for (state, sample) in izip!(&si[0..n_samples], samples[0..n_samples].iter().copied()) {
        ...
        let is_alive = state.instance_state == dds_instance_state_DDS_IST_ALIVE;
